    ...
    h("div", { key: "row", className: "flex items-start justify-between gap-3" }, [
      h("div", { key: "l", className: "min-w-0 flex-1" }, [
        h("div", { key: "h", className: "flex items-center gap-2" }, [
          h("span", { key: "n", className: "text-sm font-semibold text-gray-900 truncate" }, artist.handle),
        ...
        h("p", { key: "r", className: "text-xs text-gray-500 mt-0.5" }, [
          artist.realName, " · ", artist.genre
        ]),
        h("div", { key: "m", className: "mt-2 flex items-center gap-3 text-xs text-gray-500" }, [
          ...
          artist.email && h("span", { key: "d2", className: "text-gray-300" }, "·"),
          artist.email && h("span", { key: "e", className: "text-gray-700 truncate" }, artist.email)
        ])
      ]),
      h("span", {
        key: "chk",
...
function DemoUploader({ demo, onChange, watermark, onWatermarkChange }) {
  ...
  if (!demo) {
    return h("div", {
      ...
      h("div", {
        key: "ic",
        className: "mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-white text-gray-400 shadow-sm"
      ...
      h("p", { key: "t", className: "mt-3 text-sm font-medium text-gray-900" }, "Перетягніть біт сюди"),
      h("p", { key: "h", className: "mt-1 text-xs text-gray-500" }, "MP3 / WAV / M4A · клацніть для вибору файлу")
    ]);
  }

  const set = (patch) => onChange({ ...demo, ...patch });

  return h("div", { className: "rounded-xl border border-gray-200 bg-white p-4 space-y-4" }, [
    h("div", { key: "row", className: "flex items-start gap-3" }, [
      ...
      h("div", { key: "i", className: "min-w-0 flex-1" }, [
        h("p", { key: "n", className: "text-sm font-semibold text-gray-900 truncate" }, demo.name),
        h("p", { key: "s", className: "text-xs text-gray-500" }, fmtBytes(demo.size))
      ]),
      h("button", {
        key: "x",
        type: "button",
    ...
    h("label", {
      ...
      h("span", { key: "t" }, [
        ...
        h("span", { key: "h", className: "text-gray-400" }, " — «НеоФлюкс біт» кожні ~10с (як на BeatStars).")
      ])
    ]),
...
function MetaInput({ label, value, onChange, placeholder, type = "text" }) {
  return h("div", null, [
    h("label", { key: "l", className: "block text-[10px] uppercase tracking-wider text-gray-400 mb-1" }, label),
    h("input", {
      key: "i",
      type,
      value,
...
function MessageReview({ artist, demo, onSend, onBack, onRemove, forceSent }) {
  ...
  return h("div", {
    ...
    h("div", { key: "h", className: "flex items-center justify-between gap-3 px-4 py-3 border-b border-gray-100 bg-gray-50/60" }, [
      h("div", { key: "l" }, [
        h("p", { key: "t", className: "text-sm font-semibold text-gray-900" }, artist.handle),
        h("p", { key: "s", className: "text-xs text-gray-500" }, [
    ...
    sent || forceSent
      ? h("div", { key: "s", className: "flex items-center gap-3 px-4 py-6" }, [
          ...
          h("div", { key: "t" }, [
            h("p", { key: "1", className: "text-sm font-semibold text-gray-900" }, "Надіслано"),
            h("p", { key: "2", className: "text-xs text-gray-500" }, "Лід додано в історію зі статусом Contacted.")
          ])
        ])
      : h("div", { key: "b", className: "p-4 space-y-3" }, [
          h("div", { key: "subj" }, [
            h("label", { key: "l", className: "block text-[10px] uppercase tracking-wider text-gray-500 mb-1" }, "Тема"),
            h("input", {
              key: "i",
              type: "text",
              value: subject,
          ...
          h("div", { key: "body" }, [
            h("label", { key: "l", className: "block text-[10px] uppercase tracking-wider text-gray-500 mb-1" }, "Текст листа"),
            h("textarea", {
              key: "t",
              value: body,
              rows: 10,
...
function SmtpConfig({ value, onChange }) {
  ...
  if (value && !open) {
    return h("div", { className: "flex flex-wrap items-center justify-between gap-3 rounded-lg border border-green-200 bg-green-50 px-4 py-3" }, [
      h("div", { key: "l", className: "flex items-center gap-2 text-sm" }, [
        h("span", { key: "i", className: "flex h-6 w-6 items-center justify-center rounded-full bg-green-600 text-white" },
          h(Check, { className: "w-3.5 h-3.5" })
        ),
        h("div", { key: "t" }, [
          h("div", { key: "1", className: "font-medium text-gray-900" }, value.from || value.user),
          h("div", { key: "2", className: "text-xs text-gray-500" }, `${value.host}:${value.port} · ${PROVIDERS.find((p) => p.id === value.provider)?.label || "SMTP"}`)
        ])
      ]),
      h("button", {
        key: "b",
  ...
  return h("div", { className: "space-y-3" }, [
    h("div", { key: "prov", className: "grid gap-2 sm:grid-cols-2" },
      PROVIDERS.map((p) => h("button", {
        ...
        h("div", { key: "l", className: "font-medium text-gray-900" }, p.label),
        h("div", { key: "h", className: "text-xs text-gray-500" }, p.host || "Налаштувати вручну")
      ]))
    ),
    ...
    h("div", { key: "a", className: "flex items-center justify-between" }, [
      h("p", { key: "h", className: "text-[11px] text-gray-400" }, "Дані зберігаються локально в браузері і ніколи не виходять на наші сервери."),
...
function BeatOutreach({ onSent }) {
  ...
  return h("div", { className: "space-y-6" }, [
    ...
    h(StepCard, { key: "s1", n: 1, title: "Знайдіть потенційних покупців", active: true }, [
      ...
      h("p", {
        key: "hint",
        className: `mt-3 text-xs ${selected.length ? "text-gray-600" : "text-gray-400"}`
      }, selected.length
    ...
    h(StepCard, { key: "s2", n: 2, title: "Завантажте демку біта", active: selected.length > 0 }, [
      selected.length === 0
        ? h("p", { key: "e", className: "text-sm text-gray-400" }, "Спочатку виберіть артиста.")
    ...
    h(StepCard, { key: "s3", n: 3, title: "Перегляньте та надішліть повідомлення", active: !!(selected.length && demo) }, [
      !demo || !selected.length
        ? h("p", { key: "e", className: "text-sm text-gray-400" }, "Виберіть артиста і завантажте біт — згенеруємо лист під кожного.")
        : h("div", { key: "wrap", className: "space-y-3" }, [
            selected.length > 1 && h("div", {
              ...
              h("div", { key: "l", className: "text-sm text-blue-900" }, [
                h("span", { key: "b", className: "font-semibold" }, `Масове надсилання — ${pending.length}/${selected.length} адресатів`),
                h("span", { key: "s", className: "block text-xs text-blue-700/80 mt-0.5" }, "Один клік — і лист з бітом піде всім вибраним.")
              ]),
              h(PrimaryButton, {
                key: "b",
...
function StepCard({ n, title, active, children }) {
  return h("section", {
    ...
    h("header", { key: "h", className: "flex items-center gap-3 mb-4" }, [
      ...
      h("h2", { key: "t", className: "text-base font-semibold text-gray-900" }, title)
    ]),
