...
const COPY: Record<
  PageKey,
  Record<MarketingLocale, { title: string; description: string }>
> = {
  home: {
    uk: {
...
const PATHNAME_BY_PAGE: Record<PageKey, Record<MarketingLocale, string>> = {
  home: { uk: "/", en: "/", pl: "/" },
  pricing: { uk: "/pricing", en: "/pricing", pl: "/pricing" },
  webAgencies: {
...
export function marketingPageMetadata(
  ...
  const pathname = PATHNAME_BY_PAGE[page][loc];
  return buildPageMetadata({
    ...
    hreflangMap: getHreflangMap(loc, pathname),
  });
}
