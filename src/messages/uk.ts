import type { AbstractIntlMessages } from "next-intl";

const messages = {
  BrandMark: {
    homeAria: "На головну панель",
  },
  Meta: {
    title: "Flux Leads",
    description: "AI Lead Engine для B2B аутрічу",
  },
  Marketing: {
    productName: "Flux Leads",
    navAria: "Головна навігація",
    navFeatures: "Можливості",
    navPricing: "Тарифи",
    navWebAgencies: "Для веб-студій",
    ctaLogin: "Увійти",
    ctaDashboard: "Панель",
    footerTagline:
      "Lead Engine і мікро-CRM для холодного аутрічу. Від NeoFlux Software.",
    footerProduct: "Продукт",
    footerCompany: "Компанія",
    footerPolicy: "Політика конфіденційності",
    footerRights: "Усі права захищено.",
  },
  MarketingHome: {
    heroBadge: "Flux Leads · Outreach через аудит",
    heroH1: "Знаходь бізнеси зі слабкими сайтами. Перетворюй аудит у клієнтів.",
    heroSub:
      "Flux Leads допомагає веб-студіям знаходити локальні бізнеси, перевіряти їхні сайти та генерувати персоналізовані cold email на основі реальних проблем.",
    heroCta: "Почати безкоштовно",
    heroSecondary: "Можливості",
    featuresEyebrow: "Можливості",
    heroImageAlt: "Інтерфейс Flux Leads",
    featuresH2: "Один процес від пошуку до пітчу",
    featureSearchH3: "AI-пошук лідів",
    featureSearchP:
      "Ніша й місто → Gemini з Google Search grounding знаходить бізнеси з публічних джерел. Стартова точка для outbound, не «чорна база».",
    featureAuditH3: "Playwright-аудит сайту",
    featureAuditP:
      "SSL, mobile viewport, title/h1, швидкість завантаження та email enrichment — факти для пітчу «ваш сайт втрачає клієнтів».",
    featureScoreH3: "Opportunity Score",
    featureScoreP:
      "Оцінка 0–100 поєднує технічні проблеми та слабку веб-присутність — ви пишете спочатку тим, хто втрачає найбільше.",
    featureCrmH3: "Kanban CRM",
    featureCrmP:
      "Статуси New → Contacted → Won, історія листів і AI-генерація — без HubSpot для малої команди.",
    exampleEyebrow: "Приклад аудиту",
    exampleH2: "Покажи конкретну причину для першого контакту",
    exampleIntro:
      "Приклад локального ліда показує проблеми сайту, score і кут для листа ще до того, як ти витрачаєш час на outreach.",
    exampleBusinessLabel: "Бізнес",
    exampleBusiness: "Локальна стоматологія у Варшаві",
    exampleIssuesLabel: "Знайдені проблеми сайту",
    exampleIssue1: "Немає чіткого H1",
    exampleIssue2: "Повільне завантаження на mobile",
    exampleIssue3: "Не знайдено контактний email",
    exampleIssue4: "Слабкий SEO title",
    exampleScoreLabel: "Opportunity Score",
    exampleEmailLabel: "Згенерована чернетка листа",
    exampleEmail:
      "Привіт, я помітив, що на вашому сайті є кілька проблем, які можуть знижувати довіру й кількість заявок, особливо на mobile. Я підготував короткий аудит і можу показати, що варто покращити.",
    workflowEyebrow: "Процес у продукті",
    workflowH2: "Чотири екрани, один повторюваний sales-процес",
    workflowLeadSearchTitle: "Lead Search",
    workflowLeadSearchBody:
      "Вкажіть нішу й місто та отримайте локальні бізнеси з публічних джерел.",
    workflowAuditTitle: "Website Audit",
    workflowAuditBody:
      "Перевіряйте SSL, mobile, SEO та performance проблеми до outreach.",
    workflowEmailTitle: "AI Email Draft",
    workflowEmailBody:
      "Генеруйте персоналізовані cold email на основі реальних проблем сайту.",
    workflowCrmTitle: "Kanban CRM",
    workflowCrmBody: "Ведіть лідів від New до Won без таблиць.",
    scoreEyebrow: "Opportunity Score",
    scoreH2: "Чому один лід має 82, а інший 35",
    scoreIntro:
      "Score пріоритезує бізнеси, де проблема сайту достатньо видима для релевантного sales-пітчу.",
    scoreItem1: "Технічні проблеми сайту",
    scoreItem2: "Відсутні або слабкі SEO-основи",
    scoreItem3: "Mobile usability",
    scoreItem4: "Наявність контактів",
    scoreItem5: "Релевантність бізнесу",
    scoreItem6: "Потенціал для outreach",
    audienceEyebrow: "Для кого?",
    audienceH2: "Створено для сфокусованого B2B outreach",
    builtForTitle: "Підходить для",
    builtFor1: "Фриланс веб-розробників",
    builtFor2: "Невеликих веб-студій",
    builtFor3: "SEO-спеціалістів",
    builtFor4: "Розробників landing page",
    builtFor5: "Локальних marketing agencies",
    notForTitle: "Не для",
    notFor1: "Spam campaigns",
    notFor2: "Масового scraping",
    notFor3: "Купівлі випадкових email-баз",
    notFor4: "Заміни enterprise CRM",
    emailCompareEyebrow: "Before / after",
    emailCompareH2: "Заміни generic outreach на лист із доказами",
    genericEmailTitle: "Generic cold email",
    genericEmailBody: "Привіт, я роблю сайти. Вам потрібен новий сайт?",
    fluxEmailTitle: "Flux Leads email",
    fluxEmailBody:
      "Привіт, я перевірив ваш сайт і помітив, що він повільно завантажується на mobile та не має чіткої H1/title структури. Це може знижувати довіру й ускладнювати контакт клієнтів з вами. Можу надіслати короткий план покращень, якщо актуально.",
    trustEyebrow: "Довіра",
    trustH2: "Створено NeoFlux Software",
    trustP:
      "Flux Leads створює NeoFlux Software — невелика українська engineering team, яка будує практичні інструменти для digital-бізнесів.",
    trustItem1: "Без картки на старті",
    trustItem2: "Пошук лідів з публічних джерел",
    trustItem3: "Оплата через Stripe",
    trustItem4: "Легка CRM включена",
    trustItem5: "Можна скасувати будь-коли",
    faqH2: "Питання",
    faq1Q: "Чим Flux Leads відрізняється від CRM?",
    faq1A:
      "Це Lead Engine: пошук, аудит, пріоритет і листи. CRM — легка воронка поверх лідів.",
    faq2Q: "Чи потрібен технічний досвід?",
    faq2A:
      "Ні. Вкажіть нішу та місто, запустіть аудит, відредагуйте AI-лист і надішліть.",
    faq3Q: "Скільки коштує старт?",
    faq3A: "Starter — $0, 50 лідів на місяць. Pro — $20, Agency — $60.",
    faq4Q: "Звідки беруться дані лідів?",
    faq4A:
      "Flux Leads використовує AI-пошук по публічно індексованих джерелах і зберігає корисний контекст бізнесу для перевірки. Дані варто верифікувати перед outreach.",
    faq5Q: "Flux Leads відправляє email за мене?",
    faq5A:
      "Сервіс може генерувати листи й відправляти їх через ваш SMTP. Ви відповідаєте за текст, таргетинг і compliance.",
    faq6Q: "Flux Leads сумісний із GDPR?",
    faq6A:
      "Flux Leads допомагає знаходити публічно доступну бізнес-інформацію й готувати outreach. Користувачі відповідають за дотримання локальних правил email marketing і захисту даних, зокрема GDPR/ePrivacy. Ми радимо писати лише релевантним B2B prospects, використовувати business email, де можливо, і завжди додавати opt-out.",
    faq7Q: "Чи можна використовувати в Польщі, Україні або ЄС?",
    faq7A:
      "Так, але правила B2B outreach відрізняються за країнами. Працюйте з релевантними контактами, зберігайте source trail і припиняйте контакт після opt-out.",
    faq8Q: "Що буде після досягнення місячного ліміту?",
    faq8A:
      "Створення нових лідів зупиниться до наступного reset або upgrade. Існуючі ліди, аудити й CRM записи залишаються доступними.",
    faq9Q: "Як працює Opportunity Score?",
    faq9A:
      "Вищий score означає, що бізнес має більше видимих проблем сайту і може краще підходити для outreach через аудит.",
    faq10Q: "Чи перевіряє сервіс email адреси?",
    faq10A:
      "Flux Leads може знаходити email у HTML сайту. Це confidence signal, а не гарантія deliverability.",
    faq11Q: "Чи є безкоштовний план?",
    faq11A:
      "Так. Starter включає 50 нових лідів на місяць і не потребує картки.",
    ctaH2: "Готові до системного аутрічу?",
    ctaP: "50 лідів на місяць безкоштовно. Без картки на Starter.",
    ctaButton: "Створити безкоштовний акаунт",
    ctaFluxId: "Powered by Flux ID — один акаунт для продуктів NeoFlux.",
  },
  WebAgencies: {
    h1: "Лідогенерація для веб-студій через аудит сайтів",
    intro:
      "Пітч «ваш сайт коштує заявок» працює, коли є докази. Flux Leads знаходить prospects, аудитує сайт і готує персоналізований лист — від першого контакту до закриття угоди у Kanban-воронці.",
    h2Pain: "Чому веб-студії не можуть знайти клієнтів",
    pain1: "Години ручного пошуку в Google Maps, LinkedIn і соцмережах",
    pain2: "Шаблонні листи без конкретних фактів про сайт клієнта",
    pain3: "Ліди губляться між таблицями, Notion і поштою",
    h2How: "Як працює outreach через аудит",
    step1: "AI знаходить локальні бізнеси за нішею, містом і регіоном",
    step2: "Аудит фіксує SSL, mobile, SEO і швидкість — твердий доказ",
    step3: "Opportunity Score (0-100) підказує, кому писати першим",
    step4: "AI генерує cold email з конкретними проблемами цього сайту",
    step5: "Kanban веде угоду від New, через Contacted, до Won",
    h2Example: "Приклад: локальний стоматолог у Києві зі слабким сайтом",
    exampleBody:
      "Ви вводите «стоматологія Київ» у Flux Leads. AI повертає 20 локальних бізнесів з публічних джерел. Аудит показує, що клініка «SmileLab» має сайт без SSL, повільне завантаження 6.4 сек на мобільному і немає email у хедері. Opportunity Score: 87/100. AI генерує лист: «Доброго дня, я помітив три конкретні проблеми на вашому сайті, які можуть знижувати кількість запитів від нових пацієнтів…». Ви відправляєте зі свого SMTP. Лід переходить у Contacted. Через два дні клієнт відповідає.",
    h2Manual: "Ручний outreach vs Flux Leads",
    manualTime: "8 годин на тиждень",
    manualTimeBody:
      "Ручний пошук компаній, аналіз сайту, написання листа і внесення в CRM по одному ліду.",
    fluxTime: "20 хвилин на тиждень",
    fluxTimeBody:
      "Вводите нішу + місто, AI робить аудити, драфти і пріоритизацію — ви обираєте, кому написати.",
    h2WhoFor: "Для кого Flux Leads",
    whoFor1: "Веб-студії і дизайн-агенції (1-15 людей)",
    whoFor2: "Freelance веб-розробники, що шукають клієнтів на сайти",
    whoFor3: "SEO-спеціалісти, які пропонують аудити як lead magnet",
    whoFor4: "Маркетинг-агенції, що роблять B2B cold outreach",
    h2Issues: "Які проблеми сайту виявляє Flux Leads",
    issue1: "Відсутній SSL/HTTPS — сайт незахищений, попередження браузера",
    issue2: "Немає мобільної адаптації — viewport, ламається layout",
    issue3: "Повільне завантаження — понад 3 секунди, низька конверсія",
    issue4: "Відсутній або слабкий SEO title і meta description",
    issue5: "Немає чіткого H1 або він не на першому екрані",
    issue6: "Немає контактного email у HTML/контактах",
    issue7: "Немає Open Graph і Twitter cards — слабке поширення в соцмережах",
    h2BeforeAfter: "Cold email: шаблон vs Flux Leads",
    beforeTitle: "Generic шаблон (1-2% відповідей)",
    beforeBody:
      "Доброго дня, я веб-розробник. Створюю сучасні сайти для бізнесу. Чи потрібен вам новий сайт? Можу надіслати пропозицію.",
    afterTitle: "Flux Leads лист (3-4× вищий reply rate)",
    afterBody:
      "Доброго дня, перевірив ваш сайт і помітив три конкретні проблеми: завантажується 6.4 сек на мобільному, немає SSL і немає email у футері. Це може знижувати кількість запитів від потенційних пацієнтів — особливо тих, що шукають «стоматолог Київ» з телефону. Можу надіслати короткий аудит (3 сторінки) з конкретними правками, якщо це актуально.",
    h2Faq: "FAQ для веб-студій",
    faqQ1: "Скільки лідів можу отримати на місяць?",
    faqA1:
      "Starter: 50/міс безкоштовно. Pro: 1000/міс за $20. Agency: 10 000/міс за $60. Можна стартувати зі Starter і upgrade, коли побачите, що AI справді знаходить ваших клієнтів.",
    faqQ2: "Чи відправляє Flux Leads листи за мене?",
    faqA2:
      "Так, через ваш SMTP (Gmail App Password, Hostinger, SendGrid). Sender reputation залишається ваша — ми не шлемо з платформи.",
    faqQ3: "Чи актуальні дані лідів?",
    faqA3:
      "AI-пошук використовує публічні джерела, проіндексовані на момент пошуку. Завжди перевіряйте перед відправкою — телефон, адреса, години роботи можуть застаріти.",
    faqQ4: "Чи можна використовувати поза Україною?",
    faqA4:
      "Так. Flux Leads працює з будь-яким регіоном — задаєте місто/країну в полі region. UA, EN, PL — повна локалізація інтерфейсу і згенерованих повідомлень.",
    ctaTitle: "Почніть з 50 безкоштовних лідів на місяць",
    ctaBody:
      "Без карти. Без обмежень часу на Starter. Upgrade, коли будете готові.",
    cta: "Спробувати безкоштовно",
    relatedHeading: "Дивіться також",
    relatedPricing: "Тарифи — Starter / Pro / Agency",
    relatedFindClients: "Як знайти клієнтів на сайти",
    relatedLeadGen: "Лідогенерація для веб-студій (детально)",
    relatedAuditTool: "Інструмент аудиту локальних сайтів",
  },
  LanguageSwitcher: {
    label: "Мова",
    uk: "Українська",
    en: "English",
    pl: "Polski",
  },
  ThemeToggle: {
    toggle: "Перемкнути тему",
    switchToLight: "Світла тема",
    switchToDark: "Темна тема",
  },
  AuthHeader: {
    productName: "Flux Leads",
    admin: "Адмін",
    autopilot: "Автопілот",
    fluxPromote: "Flux Promote",
    settings: "Налаштування",
    signOut: "Вийти",
    fallbackUser: "Користувач",
    openProfile: "Профіль і налаштування",
  },
  FluxPromote: {
    title: "Flux Promote",
    subtitle: "Просувай проєкти NeoFlux через цільовий PR-аутріч.",
    selectCompany: "Оберіть компанію",
    selectProject: "Оберіть проєкт",
    targetAudiences: "Цільові аудиторії",
    suggestedKeywords: "Рекомендовані пошукові ключі",
    projectFitScore: "Оцінка відповідності проєкту",
    suggestedAngle: "Рекомендований PR-кут",
    whyThisTarget: "Чому ця ціль підходить?",
    generatePrMessage: "Згенерувати PR-повідомлення",
    shortDm: "Коротке DM",
    communityComment: "Коментар для спільноти",
    launchPost: "Пост про запуск",
    partnerPitch: "Пропозиція партнерства",
    sellerInvitation: "Запрошення продавця",
    betaTesterInvitation: "Запрошення beta-тестера",
    safetyWarning:
      "Перевіряйте кожне повідомлення перед відправкою. Поважайте правила платформ, правила спільнот і opt-out.",
    createCampaign: "Створити PR-кампанію",
    campaigns: "Кампанії",
    noCampaigns: "Flux Promote кампаній ще немає.",
    campaignCreated: "Flux Promote кампанію створено.",
    searchTargets: "Пошук PR-цілей",
    searchPlaceholder:
      "Приклад: Unity developers posting about AI tooling on GitHub and X",
    searchCta: "Знайти цілі",
    addTarget: "Додати ціль вручну",
    saveTarget: "Зберегти ціль",
    targetSaved: "Ціль збережено.",
    targetsSaved: "Збережено цілей: {count}.",
    noTargets: "PR-цілей ще немає. Запустіть пошук або додайте вручну.",
    messageDraftTabs: "Вкладки чернеток",
    messageGenerated: "PR-чернетку згенеровано для ручної перевірки.",
    working: "Працюю...",
    genericError: "Щось пішло не так.",
  },
  DatabaseConfigBanner: {
    missingTitle: "База даних не підключена на цьому середовищі",
    queryTitle: "Не вдалося зчитати ліди з бази",
    vercelSteps:
      "Для Vercel відкрийте Project → Settings → Environment Variables і додайте DATABASE_URL та DIRECT_URL (як у локальному .env / Supabase). Потім зробіть Redeploy.",
    schemaSteps:
      "Одноразово застосуйте схему до продакшен-БД: npx prisma db push з production DATABASE_URL.",
    logsHint:
      "Детальні логи: Vercel → Deployments → виберіть деплой → Runtime Logs / Functions.",
  },
  ActionErrors: {
    unauthorized: "Не авторизовано",
    notFound: "Не знайдено",
    leadNotFound: "Лід не знайдено",
    userNotFound: "Користувача не знайдено",
    missingLeadId: "Відсутній id ліда",
    queryCityRequired: "Потрібні ніша та місто",
    limitReached:
      "Ліміт плану {plan} вичерпано ({used}/{limit}). Оновіть тариф на /pricing.",
    noWebsite: "У ліда немає сайту для аудиту",
    planAuditRequired:
      "Аудит сайту доступний на плані Pro і вище. Оновіть тариф на /pricing.",
    bulkAuditRequired: "Масовий аудит доступний на плані Pro і вище.",
    updateStatusFailed: "Не вдалось оновити статус",
    genericFailed: "Щось пішло не так",
    unexpected: "Сталася неочікувана помилка",
    invalidStatus: "Невалідний статус: {status}",
    saveFailed: "Не вдалось зберегти",
    emptySubject: "Тема не може бути порожньою",
    emptyBody: "Текст повідомлення не може бути порожнім",
    missingArtist: "Відсутні дані артиста",
    noArtistEmail: "У артиста немає валідного email — оберіть інший канал.",
    missingPayload: "Відсутній payload",
    invalidPayloadJson: "Невалідний JSON у payload",
    stripeInvalidPrice: "Невалідний Stripe price/product ID",
    stripeNoSessionUrl: "Stripe не повернув URL сесії",
    stripeNoPortalUrl: "Stripe не повернув URL порталу",
    stripeApiError: "Помилка Stripe API",
    stripeCustomerMissing: "Customer ID не знайдено",
    baseUrlMissing: "NEXT_PUBLIC_BASE_URL не налаштовано",
    geminiKeyMissing:
      "GEMINI_API_KEY не налаштовано. Додайте ключ у .env (локально) або змінні Vercel.",
    aiNoText: "AI не повернув тексту",
    aiGenerateFailed: "Помилка генерації",
    searchPromptRequired: "Введіть запит для пошуку",
    searchPromptTooLong: "Запис занадто довгий — максимум {max} символів.",
    beatSearchEmpty: "Не знайдено артистів за цим запитом. Спробуйте інший жанр.",
    beatSearchNoData: "AI не повернув даних. Спробуйте ще раз або змініть запит.",
    universalParseFailed:
      "Не вдалося розпарсити відповідь AI. Спростіть або скоротіть запит.",
    universalNoValidRows: "AI не повернув жодного валідного запису з полем name.",
    universalTimeout:
      "Час очікування вичерпано. Скоротіть запит або спробуйте ще раз.",
    importEmpty: "Немає рядків для імпорту",
    importTooMany: "Занадто багато рядків (макс. {max})",
    bulkNoSelection: "Не вибрано жодного ліда",
    telegramNotConnected: "Telegram не підключено",
    fluxPromoteForbidden: "Flux Promote недоступний для вашого акаунта",
  },
  UsageMeter: {
    unlimited: "Безліміт лідів за тарифом",
    remaining:
      "{remaining} із {limit} нових лідів цього місяця ({used} уже використано)",
    exhausted:
      "Ліміт плану вичерпано ({used}/{limit}). Оновіть тариф або зачекайте до наступного періоду.",
    upgradeCta: "Оновити тариф",
    upgradeHint: "Більше лімітів і функцій — у тарифах Flux Leads.",
  },
  ModeTabs: {
    localLabel: "Локальний бізнес",
    localHint: "AI + Google · сайти й телефони з відкритих джерел",
    beatsLabel: "Виконавці для бітів",
    beatsHint: "SoundCloud · YouTube · Instagram",
    universalLabel: "Універсальний AI-пошук",
    universalHint: "Gemini + Google Search · OSINT",
    promoteLabel: "Flux Promote",
    promoteHint: "Admin · PR-аутріч для проєктів NeoFlux",
  },
  Home: {
    title: "Flux Leads",
    subtitleBeats: "AI-пошук артистів, які купують біти",
    subtitleUniversal: "Універсальний AI-пошук лідів (Gemini + Google Search)",
    subtitleLocal:
      "Пошук компаній за нішею та містом через Gemini і Google Search",
    tableTitleBeats: "Знайдені виконавці",
    tableTitleUniversal: "Універсальні ліди",
    tableTitleLocal: "Останні ліди",
    emptyTitleBeats: "Виконавців ще немає",
    emptyTitleOther: "Лідів ще немає",
    emptyHint: "Використайте форму вище, щоб почати пошук",
    colArtist: "Артист",
    colName: "Назва",
    colCompany: "Компанія",
    colGenre: "Жанр / Платформа",
    colDesc: "Опис (AI)",
    colCategory: "Категорія / Локація",
    colProfile: "Профіль",
    colSite: "Сайт / посилання",
    colStatus: "Статус",
    colAudience: "Аудиторія",
    colAudit: "Аудит",
    boardTitle: "Воронка лідів",
    boardSubtitle:
      "Усі режими на одній дошці: перетягніть картки між колонками, щоб змінити статус. Стрілка на картці відкриває сторінку ліда.",
  },
  LeadViewToggle: {
    ariaLabel: "Спосіб перегляду лідів",
    table: "Таблиця",
    board: "Дошка",
  },
  Kanban: {
    empty: "Лідів ще немає — створіть або імпортуйте.",
    funnelTotal: "Усього на дошці: {count}",
    funnelDistribution: "Розподіл за статусами",
    funnelHint:
      "Перетягніть картку в іншу колонку — статус збережеться автоматично. До 200 останніх лідів.",
    dropHere: "Перетягніть сюди",
    updateFailed: "Не вдалося оновити статус. Спробуйте ще раз.",
    openLead: "Відкрити лід",
    status: {
      New: "Нові",
      Qualified: "Кваліфіковані",
      Contacted: "На звʼязку",
      Replied: "Відповіли",
      Won: "Закрито",
      Lost: "Втрачено",
      "Do not contact": "Не контактувати",
    },
  },
  CsvUploader: {
    title: "Імпорт з CSV",
    subtitle:
      "Завантажте власну базу — ліди потраплять у режим UNIVERSAL без скрапінгу.",
    dropOrClick: "Перетягніть CSV або клікніть, щоб вибрати файл",
    expectedColumns: "Очікувані колонки: companyName, website, email, phone, notes",
    parsing: "Парсинг файлу...",
    importing: "Імпортую...",
    imported: "Імпортовано: {imported} (пропущено {skipped})",
    importFailed: "Не вдалося імпортувати",
    parseError: "Помилка парсингу",
    notCsv: "Файл має бути .csv",
    noRows: "У файлі не знайдено валідних рядків",
  },
  LeadsTableSection: {
    selected: "Вибрано: {count}",
    selectAll: "Вибрати все",
    clear: "Скинути",
    auditSelected: "Аудит вибраних",
    changeStatus: "Змінити статус…",
    delete: "Видалити",
    confirmDelete:
      "Видалити {count} лідів безповоротно? Прив’язані аудити та повідомлення також зникнуть.",
    auditDone: "Аудит завершено: {ok} з {total}",
    statusChanged: "Статус оновлено для {count} лідів → {status}",
    deleted: "Видалено {count} лідів",
    genericError: "Не вдалося виконати дію",
  },
  LeadTableRow: {
    noBooking: "немає бронювання",
    noLink: "Немає",
    followers: "{count} фоловерів",
    seekingType: "Шукає type beats",
    mobileBad: "Mobile ✗",
    mobileOk: "Mobile ✓",
    sslBad: "SSL ✗",
    sslOk: "SSL ✓",
    perf: "Perf {score}",
    auditIssues: "{count} проблем",
    auditPending: "Без аудиту",
  },
  FirstSearch: {
    eyebrow: "Перший пошук",
    title: "Яких клієнтів хочете знайти?",
    subtitle:
      "Почніть з однієї ніші, одного міста й однієї послуги. Flux Leads створить першу кампанію та збере перші ліди у workspace.",
    badge: "1-2 хв налаштування",
    niche: "Ніша",
    city: "Місто",
    service: "Ваша послуга",
    language: "Мова",
    submit: "Знайти перші 10 лідів",
    busy: "Шукаю...",
    searching: "Шукаю у відкритих джерелах...",
    success: "Додано {count} лідів. Далі запустіть аудит.",
    empty: "Лідів не знайдено. Спробуйте іншу нішу або місто.",
    error: "Не вдалося запустити перший пошук.",
    languageOptions: {
      Polish: "Польська",
      English: "Англійська",
      Ukrainian: "Українська",
    },
  },
  LeadFilters: {
    title: "Фільтри якості лідів",
    subtitle: "Спочатку працюйте з лідами, які вже готові до outreach.",
    clear: "Скинути фільтри",
    items: {
      hasEmail: "Є email",
      score70: "Score 70+",
      notContacted: "Не контактували",
      hasWebsite: "Є сайт",
      hasPhone: "Є телефон",
      websiteIssues: "Є проблеми сайту",
      contacted: "Contacted",
      replied: "Replied",
      noEmail: "Email не знайдено",
    },
  },
  Campaigns: {
    title: "Кампанії",
    subtitle: "Групування за нішею та містом, щоб пошуки не змішувалися в один список.",
    empty: "Перша кампанія зʼявиться після локального пошуку.",
    leads: "Ліди",
    contacted: "Контакт",
    replies: "Відповіді",
    won: "Won",
    goal: "Ціль",
    language: "Мова",
  },
  ScraperForm: {
    nicheLabel: "Ніша / тип бізнесу",
    nichePlaceholder: "Напр.: Стоматологія, автосервіс...",
    cityLabel: "Місто",
    cityPlaceholder: "Напр.: Черкаси",
    submitIdle: "Пошук",
    submitBusy: "Збираю дані…",
    helperAi:
      "Шукаємо у відкритому вебі через AI (Google Search grounding) — зазвичай 10–40 с.",
    toastAdded: "Додано {count} нових лідів.",
    toastEmpty:
      "За цим запитом не знайдено жодної компанії в доступних джерелах. Спробуйте іншу нішу, місто або формулювання.",
    toastDup: "Усі знайдені компанії вже є у вашій таблиці.",
    toastErr: "Помилка: {message}",
    blockedTitle: "Ліміт плану вичерпано",
    blockedBody:
      "Ви використали всі нові ліди цього місяця для плану {plan}. Оновіть тариф або зачекайте до наступного періоду.",
    blockedCta: "Переглянути тарифи",
  },
  UniversalOutreach: {
    label: "Запит до Gemini",
    promptLabel: "Опишіть, кого шукаєте",
    placeholder:
      "Опишіть ідеального ліда: нішу, регіон, розмір бізнесу, ознаки з відкритих джерел…",
    promptPlaceholder:
      "Наприклад: «Знайди 5 SaaS стартапів у сфері логістики в Польщі з публічним сайтом» або «Контакти HR-директорів українських IT-компаній 200+ людей»",
    submitIdle: "Знайти лідів",
    submitBusy: "Шукаю…",
    ctaScan: "Запустити AI-сканер",
    busyInternet: "AI аналізує інтернет…",
    googleWait: "Зачекайте — виконується пошук у Google…",
    hint: "Модель використовує Google Search grounding — результати залежать від відкритих сторінок.",
    toastOk: "Додано {count} нових лідів.",
    toastDup: "За цим запитом нових унікальних лідів не знайдено.",
    toastErr: "Помилка: {message}",
    searchFailed: "Пошук не вдався",
    networkError:
      "Зв'язок з сервером перервався або запит занадто довгий: {message}. Спробуйте скоротити запит і натиснути знову.",
    networkErrorGeneric:
      "Зв'язок з сервером перервався (часто через таймаут на дуже складному пошуку). Спростіть запит або спробуйте пізніше.",
    savedPrefix: "Збережено в CRM:",
    recordOne: "запис",
    recordMany: "записів",
    savedSuffix: "(нові та оновлені за назвою).",
    footerNote:
      "Результати зберігаються як ліди режиму «Універсальний». Перевіряйте достовірність даних перед outreach — AI може помилятися.",
    blockedTitle: "Ліміт плану вичерпано",
    blockedBody:
      "Ви використали всі нові ліди цього місяця для плану {plan}. Оновіть тариф або зачекайте до наступного періоду.",
    blockedCta: "Переглянути тарифи",
  },
  Login: {
    title: "Flux Leads",
    subtitle:
      "Увійдіть, щоб відкрити панель лідів та інструменти outreach.",
    fluxCta: "Увійти через Flux ID",
    fluxFooter: "Flux ID — єдиний акаунт для всіх продуктів екосистеми Flux.",
    noAccount: "Немає акаунту?",
    register: "Зареєструватися",
    errors: {
      not_configured:
        "Flux ID не налаштований на сервері. Адміністратор має задати змінні FLUX_*.",
      state_mismatch:
        "Сесія входу застаріла або підроблена. Спробуйте ще раз.",
      missing_code_or_state:
        "Flux ID не повернув потрібних параметрів. Спробуйте знову.",
      token_exchange_failed:
        "Не вдалося обміняти код на токен. Перевірте FLUX_CLIENT_SECRET.",
      userinfo_failed:
        "Flux ID повернув помилку при отриманні профілю. Спробуйте пізніше.",
      invalid_subject:
        "Flux ID повернув некоректний ідентифікатор користувача.",
      db_failure:
        "Не вдалося зберегти користувача в базі. Спробуйте ще раз.",
      access_denied: "Ви не дали згоду на вхід через Flux ID.",
      fallback: "Помилка входу: {code}",
    },
  },
  Settings: {
    back: "Назад до робочого простору",
    title: "Налаштування",
    subtitle:
      "Підключи власний SMTP-сервер, щоб Flux Leads відправляв листи від твого імені. Автоматична розсилка з додатку працює лише на email; для Instagram, Telegram та інших каналів треба писати самостійно.",
    smtpSectionTitle: "SMTP / Bring Your Own Email",
    smtpSectionSubtitle:
      "Підтримує будь-який SMTP — Gmail (App Password), Hostinger, SendGrid, Mailgun, AWS SES. Це лише для пошти; інші способи зв'язку додаток не надсилає автоматично.",
  },
  SmtpSettingsForm: {
    host: "Host",
    hostHint: "напр. smtp.gmail.com або smtp.hostinger.com",
    hostPlaceholder: "smtp.example.com",
    port: "Port",
    portHint: "465 для SSL, 587 для STARTTLS",
    username: "Username",
    usernameHint: "зазвичай — твій email",
    password: "Password",
    passwordHint:
      "App password або SMTP-токен. Зберігається зашифровано (AES-256-GCM).",
    passwordPlaceholderExisting: "Не міняти існуючий",
    passwordPlaceholderNew: "App password…",
    fromEmail: "From Email",
    fromEmailHint: "адреса відправника в листах",
    fromEmailLabelPlatform: "Email для відповідей",
    fromEmailLabelCustom: "From Email",
    fromEmailHintPlatform:
      "клієнти відповідатимуть саме сюди — це твій робочий інбокс",
    fromEmailHintCustom: "адреса в заголовку From: твоїх листів",
    fromName: "Ім'я відправника",
    fromNameHint: "необов'язково — показується замість email",
    fromNameHintPlatform:
      "як підписатись в листах; показується замість email",
    fromNameHintCustom: "опційно — показується замість email",
    fromNamePlaceholder: "Sales Team",
    save: "Зберегти",
    saving: "Зберігаю…",
    success: "Збережено. SMTP готовий до відправок.",
    successPlatform:
      "Збережено. Тепер можна слати листи через платформу — відповіді приходитимуть на твій email.",
    successCustom: "Збережено. Власний SMTP готовий до відправок.",
    errorSave: "Не вдалось зберегти налаштування.",
    tablistLabel: "Режим відправки email",
    platformBannerTitle: "Листи летять через сервер Flux Leads",
    platformBannerBody:
      "Нічого не налаштовуєш — просто натискаєш Send. Всі відповіді від клієнтів приходитимуть тобі на email, який ти вкажеш нижче (Reply-To).",
    customBannerTitle: "Власний SMTP",
    customBannerBody:
      "Підтримує будь-який SMTP — Gmail (App Password), Hostinger, SendGrid, Mailgun, AWS SES. Лист буде з адреси, яку ти вкажеш — на ній будується репутація відправника.",
    footerSignature:
      "Кожен лист, відправлений з Flux Leads, отримує невеликий підпис із посиланням на сервіс.",
    errors: {
      generic: "Не вдалось зберегти налаштування",
      host: "Host обов'язковий",
      port: "Невалідний port (1-65535)",
      user: "Username обов'язковий",
      email: "Невалідний From Email",
    },
    modePlatform: "Простий режим",
    modePlatformSub: "Рекомендовано",
    modeCustom: "Розширений режим",
    modeCustomSub: "Власний SMTP",
    modeTest: "Тест",
    modeTestSub: "Лист і вкладення MP3",
    testReadyTitle: "Тестова відправка",
    testReadyBody:
      "Лист піде через {mode} — те, що збережено зараз у базі. Якщо обрати файл нижче, вкладення піде так само, як у режимі «Біти».",
    testModePlatform: "платформений SMTP (Простий режим)",
    testModeCustom: "ваш власний SMTP (Розширений режим)",
    testIncompleteTitle: "Конфіг не збережено повністю",
    testIncompleteBody:
      "Бракує: {missing}. Перейдіть на вкладку «{tab}», заповніть поля і натисніть «Зберегти». Кнопку нижче все одно можна натиснути — сервер покаже точну причину.",
    testTabSimple: "Простий",
    testTabAdvanced: "Розширений",
    testRecipientLabel: "Email одержувача",
    testRecipientHint:
      "Свій email або будь-яка адреса для перевірки доставки",
    testAttachmentLabel: "Аудіо для вкладення (опційно)",
    testAttachmentHint:
      "MP3 / WAV / M4A / FLAC / OGG, до 10 MB — як у «Надіслати на Email» для бітів. Якщо файл не обрано, піде звичайний текстовий тест.",
    testAttachmentClear: "Прибрати файл",
    testFooterNote:
      "Тестовий лист має підпис Flux Leads — як і outreach-листи.",
    testSubmit: "Надіслати тестовий лист",
    testSubmitWithFile: "Надіслати тест з вкладенням",
    testSubmitPending: "Відправляю…",
    testSubmitWithFilePending: "Відправляю з файлом…",
    testSuccess: "Тестовий лист відправлено на {email}. Перевірте інбокс (і Спам).",
    testSuccessWithFile:
      "Тестовий лист з аудіо-вкладенням відправлено на {email}. Перевірте вкладення в клієнті та Спам.",
    testErrorRecipient: "Введіть email одержувача",
    testErrorPickFile: "Оберіть аудіо-файл для тесту з вкладенням",
    testErrorNoSmtpHint:
      "Перейдіть на вкладку «{tab}», заповніть поля і натисніть «Зберегти».",
    testErrorPlatformHint:
      "Це налаштовує адмін сервера (PLATFORM_SMTP_* у .env). Або увімкніть «Розширений режим» і вкажіть свій SMTP.",
  },
  Pricing: {
    back: "Назад до робочого простору",
    title: "Тарифні плани",
    subtitle: "Виберіть план під ваш обсяг outreach. Можна оновити будь-коли.",
    current: "Поточний план:",
    perMonth: "/міс",
    paymentNote:
      "Захищена оплата через Stripe. Скасувати або змінити план можна будь-коли в розділі «Керувати підпискою».",
    popular: "Найпопулярніший",
    leadsLine:
      "{formatted} нових лідів{perMonthSuffix}",
    perMonthSuffix: "/міс",
    unlimitedWord: "Безліміт",
    currentPlan: "Поточний план",
    upgradeNow: "Оновити зараз",
    plans: {
      STARTER: {
        tagline: "Безкоштовно",
        description:
          "Спробувати продукт і зрозуміти, як працює пошук лідів.",
        cta: "Почати безкоштовно",
        highlights: {
          localSearch: "Локальний пошук (AI + Google)",
          simpleAi: "Спрощена AI-генерація листів",
        },
      },
      PRO: {
        tagline: "Найпопулярніший",
        description:
          "Для фрилансерів і малих команд, які роблять outreach щодня.",
        cta: "Оновити до Pro",
        highlights: {
          fullAudit: "Повний аудит сайтів (SSL, mobile, performance)",
          emailEnrichment: "Email enrichment з HTML",
          unlimitedAi: "Безлімітна AI-генерація листів (Gemini)",
        },
      },
      AGENCY: {
        tagline: "High-volume",
        description:
          "Для агенцій, які працюють з кількома нішами й хочуть конкретний місячний ліміт із fair use.",
        cta: "Запустити Agency plan",
        highlights: {
          allPro: "Усі функції Pro",
          csvExport: "Повний експорт у CSV",
        },
      },
    },
    checkoutLoading: "Перенаправляємо…",
    checkoutSuccessTitle: "Дякуємо за підписку!",
    checkoutSuccessBody:
      "Ваші ліміти оновляться протягом декількох хвилин.",
    checkoutSuccessClose: "Закрити",
    manageSubscription: "Керувати підпискою",
    portalLoading: "Відкриваємо портал…",
  },
  AdminUsers: {
    title: "Користувачі",
    subtitle:
      "Зміни плану застосовуються миттєво. Лічильник лідів скидається на 0 при зміні плану.",
    statTotalUsers: "Всього юзерів",
    statActiveHint: "{count} активних за 30д",
    statActiveScrapers: "Активні скрапери",
    statActiveScrapersHint: "входили за останні 30 днів",
    statLeads: "Лідів у системі",
    statLeadsHint: "усі юзери разом",
    statRevenue: "Revenue / міс",
    statRevenueHint: "симульовано з PRO+AGENCY",
    statNew24h: "Нові акаунти за 24 год",
    statNew24hHint: "за датою створення запису User (Flux ID)",
    statNew7d: "Нові за 7 днів",
    statNew7dHint: "останні 7 діб",
    statNew30d: "Нові за 30 днів",
    statNew30dHint: "останні 30 діб",
    loadUsersErr: "Не вдалося завантажити користувачів: {message}",
  },
  AdminLayout: {
    backToWorkspace: "Робочий простір",
    centerTitle: "Intelligence Center",
    navUsers: "Користувачі",
  },
  AdminOverview: {
    placeholder: "Майбутній огляд платформи.",
  },
  LeadDetail: {
    back: "Назад до списку",
    scoreHeading: "Opportunity Score",
    scoreTitle: "Пріоритет ліда для продажу",
    scoreBadgeHigh: "Високий",
    scoreBadgeMedium: "Середній",
    scoreBadgeLow: "Низький",
    scoreLabel: {
      high: "Високий потенціал для продажу послуг",
      medium: "Помірний потенціал — варто перевірити",
      low: "Низький пріоритет",
    },
    scoreHintBeats:
      "Бал враховує сигнал «шукає type beats», наявність email і розмір аудиторії на платформі.",
    scoreHintLocalNoAudit:
      "Бал з AI-сигналів пошуку: відсутність сайту, відсутність онлайн-бронювання та помічені «болі» у відгуках і сніпетах.",
    scoreHintUniversalNoAudit:
      "Бал з email та наявності сайту. Після аудиту сайту врахуються технічні сигнали.",
    scoreBreakdownTitle: "Чому такий score",
    whyThisLeadTitle: "Чому це хороший lead?",
    suggestedAngleTitle: "Suggested outreach angle",
    sourceUrl: "Джерело",
    websiteUrl: "Website URL",
    contactPageUrl: "Contact page URL",
    lastChecked: "Остання перевірка",
    campaignOffer: "Офер",
    campaignLanguage: "Мова",
    contactsTitle: "Контакти",
    channelsOne: "канал",
    channelsMany: "канали",
    realName: "Реальне ім'я",
    genre: "Жанр",
    platform: "Платформа",
    audience: "Аудиторія",
    followersFmt: "{count} фоловерів",
    typeBeats: "Type beats",
    typeYes: "Так",
    typeNo: "Ні",
    seekingBadge: "Шукає type beats",
    notSeekingPublic: "Не шукає публічно",
    channelsHeading: "Канали зв'язку",
    noChannels: "Каналів зв'язку не знайдено.",
    universalDesc: "Опис (AI)",
    site: "Сайт",
    email: "Email",
    phone: "Телефон",
    socialHeading: "Соцмережі та посилання",
    noSocialExtra: "Додаткових соцмереж у записі немає.",
    category: "Категорія",
    city: "Місто",
    notProvided: "Не вказано",
    auditTitle: "Аудит сайту",
    auditEmptyTitle: "Аудит ще не проводився",
    auditEmptyHintWeb:
      "Запустіть аналіз сайту, щоб побачити Performance Score.",
    auditEmptyHintNoSite: "У ліда відсутній сайт — аудит неможливий.",
    issuesHeading: "Знайдені проблеми",
    issuesNone: "Проблем не знайдено",
    ssl: "SSL",
    mobile: "Mobile-friendly",
    systemInfoTitle: "Системна інформація",
    systemSource: "Джерело",
    systemPlatform: "Платформа",
    created: "Створено",
    updated: "Оновлено",
    messagesTitle: "Історія повідомлень",
    messagesEmptyTitle: "Поки що немає надісланих листів",
    messagesEmptyHint:
      "Згенеруйте та збережіть AI-пропозицію вище, щоб почати історію.",
    replyNoReply: "Без відповіді",
    replyReplied: "Відповів",
    replyInterested: "Зацікавлений",
    replyBounced: "Відхилено",
    replyStatusEditTitle: "Змінити статус відповіді",
    replyStatusUpdating: "Зберігаємо…",
    replyStatusUpdateFailed: "Не вдалося оновити статус: {error}",
    deliverySent: "Надіслано",
    deliveryFailed: "Помилка",
    deliveryDraft: "Чернетка",
    deliveryFailedTooltip: "Помилка відправки. Клікніть, щоб побачити деталі.",
    deliveryErrorTitle: "SMTP повернув помилку",
    deliveryErrorClose: "Закрити",
    demoAttach: "Демо",
    channelsUsed: "Канали:",
    sentVia: "Надіслано через:",
    currencyUsd: "${price}",
  },
  LeadStatus: {
    New: "Новий",
    Qualified: "Кваліфікований",
    Contacted: "Контакт",
    Replied: "Відповідь",
    Won: "Угода",
    Lost: "Втрачено",
    "Do not contact": "Не контактувати",
  },
  StatusPicker: {
    label: "Статус у CRM",
    ariaLabel: "Статус ліда",
    errorUpdate: "Не вдалось оновити статус",
  },
  AuditButton: {
    run: "Зробити аудит",
    running: "Аналізую…",
    refresh: "Оновити аудит",
    done: "Аудит пройдено",
    issuesOne: "{count} проблема",
    issuesFew: "{count} проблеми",
    issuesMany: "{count} проблем",
  },
  AIProposal: {
    sectionLabel: "AI Proposal",
    title: "Холодний лист під цього ліда",
    description:
      "Gemini 2.5 Flash проаналізує дані ліда та згенерує персоналізований лист на основі знайдених «болей».",
    emailOnlyNote:
      "Кнопка «Надіслати на Email» відправляє лист лише на пошту (через SMTP у налаштуваннях). У соцмережах чи месенджерах треба написати самостійно — скопіюйте текст або збережіть у CRM.",
    generate: "Згенерувати листа (AI)",
    regenerate: "Згенерувати ще раз",
    generating: "Генерую…",
    subjectLabel: "Тема листа",
    bodyLabel: "Текст листа",
    editHint: "Тему та текст можна редагувати перед збереженням.",
    copy: "Копіювати",
    copied: "Скопійовано",
    save: "Зберегти в CRM",
    saving: "Зберігаю…",
    savedState: "Збережено",
    send: "Надіслати на Email",
    sending: "Відправляю…",
    noEmailHint:
      "У ліда немає email — кнопку відправки заблоковано. Допишіть адресу, щоб надіслати лист.",
    savedBanner:
      "Повідомлення збережено, статус ліда змінено на Contacted.",
    analyzing: "AI аналізує ліда…",
    emptyState:
      "Натисніть кнопку, щоб AI створив персональний холодний лист.",
    toastSent: "Лист надіслано та збережено в історії.",
    toastNoSmtp:
      "Спершу заповніть SMTP-налаштування у профілі.",
    toastSmtpErr: "Помилка SMTP — перевірте налаштування.",
    openSettings: "Відкрити",
    generateErr: "Не вдалось згенерувати лист",
    saveErr: "Не вдалось зберегти повідомлення",
    copyErr: "Не вдалось скопіювати в буфер обміну",
    subjectPlaceholder: "Пропозиція для {company}",
    generatingShort: "Генерую…",
    savedBannerIntro: "Повідомлення збережено, статус ліда змінено на",
    rewriteLabel: "Переписати:",
    rewrite: {
      shorter: "Коротше",
      friendlier: "Дружній тон",
      formal: "Офіційний тон",
    },
    rewriteErr: "Не вдалось переписати",
    sequenceCta: "Згенерувати sequence",
    sequenceTitle:
      "Створити ланцюг із 3 листів: Pitch / Follow-up / Break-up",
    sequenceErr: "Не вдалось згенерувати ланцюжок",
  },
  OnboardingTour: {
    progressText: "{{current}} з {{total}}",
    helpButton: "Як це працює",
    next: "Далі",
    back: "Назад",
    done: "Готово",
    helpTitle: "Показати огляд інтерфейсу",
    helpAria: "Як це працює — короткий тур",
    steps: {
      brandTitle: "Логотип — на головну",
      brandBody:
        "Клік по марці Flux Leads відкриває основну сторінку панелі: зручно повернутися після перегляду ліда, налаштувань або іншого розділу.",
      headerTitle: "Заголовок і режим",
      headerBody:
        "Тут — назва продукту та короткий опис активного режиму: локальний бізнес (AI + Google), артисти для бітів або універсальний AI-пошук.",
      modesTitle: "Режими роботи",
      modesBody:
        "Три вкладки: локальний бізнес (AI + Google за нішею та містом), виконавці для бітів (AI + демо), універсальний пошук (Gemini + Google для довільного OSINT-запиту).",
      searchTitleBeats: "Пошук артистів (AI)",
      searchTitleUniversal: "Універсальний AI-пошук",
      searchTitleLocal: "Пошук локальних лідів",
      searchDescBeats:
        "Введіть жанр, платформу або @нік. Gemini з пошуком Google знайде реальних артистів; оберіть картки та продовжіть до демо й повідомлень.",
      searchDescUniversal:
        "Опишіть довільним текстом, кого шукати — AI просканує відкриті джерела й збере лідів у таблицю нижче.",
      searchDescLocal:
        "Введіть нішу та місто і натисніть «Пошук». Gemini з пошуком Google знайде реальні компанії з відкритих сторінок і додасть їх у таблицю нижче.",
      demoTitle: "Демо біта",
      demoBody:
        "Після вибору артистів завантажте трек: превʼю, водяний знак (у браузері), метадані для AI-листа (BPM, тональність, ціна). SMTP для email налаштовується один раз у розділі «Налаштування».",
      msgTitle: "Повідомлення та канали",
      msgBody:
        "Щоб розіслати текст разом із файлом біта: використайте «Надіслати на Email» — один раз налаштуйте SMTP у «Налаштуваннях», і додаток сам прикріпить демо до листа (потрібен публічний email артиста). У Instagram, Telegram, SoundCloud тощо додаток не може вкласти файл: кнопки каналів лише відкривають профіль або чат і копіюють текст — відкрийте діалог і прикріпіть той самий файл уручну, якщо месенджер це дозволяє. «Зберегти в CRM» зберігає тему, текст і метадані демо в базі без відправки; аудіо на сервер не завантажується.",
      tableTitleBeats: "Таблиця лідів",
      tableDescBeats:
        "Останні збережені артисти з CRM. Клік по рядку відкриє картку: соцмережі, фоловери, історія повідомлень і демо.",
      tableDescUniversal:
        "Універсальні ліди з AI: опис у другій колонці, сайт і соцмережі — у картці ліда.",
      tableDescLocal:
        "Останні додані компанії. Відкрийте лід для аудиту сайту, контактів і AI-листа.",
      leadTableTitle: "Таблиця лідів",
      replayTitle: "Підказки завжди поруч",
      replayBody:
        "Кнопка «Як це працює» запускає цей тур знову. Після першого проходження він не відкривається автоматично. На головну також можна з логотипу Flux Leads або з назви вгорі сторінки.",
    },
  },
  BeatOutreach: {
    step1Title: "Знайдіть потенційних покупців",
    step2Title: "Завантажте демку біта",
    step3Title: "Перегляньте та надішліть повідомлення",
    pickArtist:
      "Виберіть одного або декількох артистів, кому надішлемо біт.",
    pickedCount: "Вибрано: {count}",
    selectFirst: "Спочатку виберіть артиста.",
    selectAndDemo:
      "Виберіть артиста і завантажте біт — згенеруємо лист під кожного.",
    flowNote:
      "Автоматична розсилка з додатку — лише на email (SMTP у Налаштуваннях; кнопка «Надіслати на Email» на картці ліда). Для Instagram, Telegram та інших варіантів зв'язку треба писати самостійно — кнопки нижче лише відкривають профіль або чат. «Зберегти в CRM» зберігає outreach у базі без відправки в месенджери.",
    searchPlaceholder:
      "Жанр, платформа, місто, ключове слово (напр. ukrainian trap soundcloud, drill rapper looking for type beats)",
    searchBusy: "Шукаю…",
    searchIdle: "Пошук",
    resultsFailed: "Пошук не вдався",
    noResults: "Нічого не знайдено — спробуйте інший запит.",
    cardPlatforms: "Платформи:",
    cardSeeking: "Шукає type beats",
    cardFollowers: "{count} фоловерів · ",
    cardPreview: "Превʼю демо",
    demoWatermark: "Водяний знак на превʼю",
    demoHintTitle: "Локальний файл для превʼю та метаданих",
    demoHintBody:
      "Файл не завантажується на сервер — лише для браузерного превʼю та підстановки в AI-лист.",
    demoDropTitle: "Перетягніть .mp3 / .wav сюди або оберіть файл",
    demoDropHint: "до {maxMb} МБ · лише в браузері",
    demoPickAnother: "Обрати інший файл",
    demoClear: "Прибрати файл",
    metaBpm: "BPM",
    metaKey: "Тональність",
    metaGenre: "Жанр",
    metaPrice: "Ціна ($)",
    bulkTitle: "Масова відправка",
    bulkPending: "Залишилось надіслати: {pending} із {total}",
    bulkCta: "Надіслати всім з черги",
    bulkWorking: "Відправляю…",
    reviewCopy: "Копіювати",
    reviewCopied: "Скопійовано",
    reviewTelegram: "Telegram",
    reviewChannels: "Канали",
    reviewSaveCrm: "Зберегти в CRM",
    reviewSaving: "Зберігаю…",
    reviewSaved: "Збережено в CRM",
    reviewSubject: "Тема",
    reviewGenerateAi: "Згенерувати AI (Gemini)",
    reviewGenerating: "Генерую…",
    limitExceeded: "Ліміт плану вичерпано",
    removeRow: "Прибрати з вибору",
    findBuyers: "Знайти покупців",
    aiSearching: "AI шукає…",
    searchFooter:
      "Пошук через Gemini AI з Google Search grounding по живих профілях SoundCloud / YouTube / Instagram / BeatStars. Зазвичай 10–20 секунд. Перевіряйте знайдені email перед відправкою.",
    idleTitle: "AI знайде реальних артистів за вашим запитом",
    idleHint:
      "Введіть жанр чи нішу й натисніть «Знайти покупців» — Gemini пошукає активні профілі з живими контактами.",
    noResultsAlt: "Нічого не знайдено. Спробуйте інший запит.",
    profileLink: "{platform} — профіль ↗",
    contactsMissing: "⚠ Контакти не знайдено — спробуй інший запит.",
    dropzoneTitle: "Перетягніть біт сюди",
    dropzoneHint: "MP3 / WAV / M4A · клацніть для вибору файлу",
    removeDemoAria: "Видалити демо",
    voiceTagLabel: "Voice-tag водяний знак",
    voiceTagHint: "— «НеоФлюкс біт» кожні ~10с (як на BeatStars).",
    metaPriceShort: "Ціна, $",
    demoBadge: "DEMO·watermarked",
    flowNoteBefore: "Автоматична розсилка з додатку — лише на email (SMTP у ",
    flowNoteSettings: "Налаштуваннях",
    flowNoteAfter:
      "; кнопка «Надіслати на Email» на картці ліда). Для Instagram, Telegram та інших варіантів зв'язку треба писати самостійно — кнопки нижче лише відкривають профіль або чат. «Зберегти в CRM» зберігає outreach у базі без відправки в месенджери.",
    errRegenerate: "Не вдалось перегенерувати",
    errClipboard: "Не вдалось скопіювати — можливо браузер блокує clipboard.",
    errSave: "Не вдалось зберегти",
    channelsAvailable: "{count} каналів зв'язку доступно",
    noPublicContacts: "Нема публічних контактів — потрібен ручний DM на {platform}",
    remove: "Прибрати",
    savedHeading: "Збережено",
    savedBody: "Лід додано в CRM зі статусом Contacted",
    sentViaSuffix: "надіслано через",
    letterBody: "Текст листа",
    regenerateAi: "Перегенерувати (AI)",
    channelsHeadingLower: "Канали зв'язку",
    openedCount: "{count} відкрито",
    clickToOpenMessenger: "Натисніть, щоб відкрити месенджер",
    open: "Відкрити",
    noPrefillHint:
      "Платформи без URL-prefill (Instagram, SoundCloud, тощо) — повідомлення скопіюється в буфер автоматично, вставте в месенджері.",
    saveHintOptionalChannels:
      "Можна зберегти одразу — тема, текст і метадані демо. Відкриті нижче канали додатково запишуться в картці ліда.",
    saveHintChannels: "Готово — фіксуємо {count} {channelsWord} у CRM.",
    channelOne: "канал",
    channelMany: "канали",
    bulkHeading: "Масове надсилання — {pending}/{total} адресатів",
    bulkAllSent: "Всім, кого вибрали, вже надіслано.",
    bulkManual: "Надішліть кожному вручну нижче — біт точно той самий.",
    tourTableTitle: "Таблиця лідів",
    emailSendTitle: "Надіслати лист з бітом",
    emailSendHint:
      "Лист піде на {email} через ваш SMTP. MP3-демо прикладеться як вкладення; лід автоматично збережеться у CRM зі статусом Contacted.",
    emailSendCta: "Надіслати на Email",
    emailSending: "Відправляю…",
    errSendEmail: "Не вдалося відправити лист.",
    emailHintNoSmtp:
      "SMTP не сконфігуровано. Відкрийте /settings і заповніть «Простий режим» або «Розширений режим».",
    emailHintPlatformNotConfigured:
      "Платформений SMTP не налаштований адміністратором (PLATFORM_SMTP_* у .env). Або переключіть свій режим у «Розширений» і вкажіть власні SMTP-креди.",
    emailHintFileTooLarge:
      "Спробуйте стиснути MP3 у бітрейт 128–192kbps або обрізати тривалість.",
  },
  Channels: {
    labels: {
      email: "Email",
      phone: "Телефон",
      website: "Сайт",
      instagram: "Instagram",
      soundcloud: "SoundCloud",
      youtube: "YouTube",
      tiktok: "TikTok",
      twitter: "X / Twitter",
      spotify: "Spotify",
      beatstars: "BeatStars",
      telegram: "Telegram",
      discord: "Discord",
      whatsapp: "WhatsApp",
    },
    hints: {
      email:
        "mailto: відкриває поштовий клієнт із заповненою темою та текстом",
      phone: "tel: для дзвінка",
      website: "Відкриває особистий сайт артиста",
      instagram:
        "Відкриває профіль (DM-prefill не підтримується — текст у буфері)",
      soundcloud:
        "Відкриває профіль (DM через клік на «Message» там)",
      youtube: "Відкриває канал (контакт через email на About)",
      tiktok: "Відкриває профіль",
      twitter: "Відкриває профіль або compose-DM, якщо знаємо ID",
      spotify: "Сторінка артиста (без DM)",
      beatstars: "Профіль артиста на BeatStars",
      telegram: "Відкриває чат через t.me/<handle>",
      discord:
        "Юзернейм для пошуку у Discord (DM-deep-link не підтримується)",
      whatsapp: "wa.me з prefill повідомлення",
    },
  },
  AdminUsersTable: {
    empty: "Користувачів ще немає.",
    colUser: "Користувач",
    colRole: "Роль",
    colPlan: "План",
    colLeads: "Ліди",
    colUsed: "Викор.",
    colLastLogin: "Останній вхід",
    noEmail: "немає email",
    genericFailed: "Не вдалося",
    email: "Email",
    role: "Роль",
    plan: "План",
    leadsUsed: "Ліди (міс.)",
    joined: "Реєстрація",
    actions: "Дії",
    save: "Зберегти",
    saving: "Зберігаю…",
    ownerNote: "OWNER призначається лише через OWNER_EMAIL у .env",
  },
  Footer: {
    brandName: "Flux Leads",
    tagline:
      "AI lead engine для веб-студій і фрілансерів. Знаходьте локальні бізнеси, аудитуйте слабкі сайти та готуйте персоналізований outreach.",
    projectsTitle: "Наші проєкти",
    accountTitle: "Акаунт",
    resourcesTitle: "Ресурси",
    companyTitle: "NeoFlux Software",
    linkFluxId: "Flux ID",
    linkMarketplace: "Flux MarketPlace",
    linkSocial: "Fluxy Social",
    linkFluxPay: "Flux Pay",
    badgeSoon: "SOON",
    linkPricing: "Тарифи",
    linkSettings: "Налаштування",
    linkLogin: "Увійти",
    linkPolicy: "Політика конфіденційності",
    linkTerms: "Умови користування",
    linkCookies: "Файли cookie",
    linkAcceptableUse: "Правила допустимого використання",
    socialTelegram: "Telegram",
    socialYoutube: "YouTube",
    socialTiktok: "TikTok",
    rightsReserved: "Усі права захищено.",
  },
  TelegramAuth: {
    title: "Telegram акаунт",
    subtitle:
      "Підключіть свій номер, щоб Автопілот міг писати лідам прямо в Telegram.",
    statusConnected: "Підключено",
    statusDisconnected: "Не підключено",
    statusStep2: "Крок 2 з 2",
    connectedPhone: "Активний номер",
    sentToday: "{count} повідомлень за сьогодні",
    appIdSuffix: " · App ID {apiId}",
    disconnect: "Відключити",
    sendCode: "Надіслати код",
    sending: "Надсилаю…",
    verify: "Підтвердити",
    verifying: "Перевіряю…",
    cancel: "Скасувати",
    codeInstruction:
      "Введіть код, який Telegram надіслав на {phone} (in-app або SMS).",
    passwordInstruction:
      "Цей акаунт використовує двофакторну автентифікацію. Введіть Telegram-пароль (cloud password).",
    passwordPlaceholder: "Cloud password",
    confirmDisconnect:
      "Відключити Telegram? Активні автопілот-конфіги з каналом Telegram перестануть слати повідомлення.",
    infoCodeSent: "Код надіслано — перевірте Telegram.",
    infoNeedsPassword: "Потрібен 2FA пароль.",
    infoConnected: "Підключено успішно.",
    infoDisconnected: "Відключено.",
    infoCredsSaved: "Ключі збережено. Тепер введіть номер телефону.",
    errPhoneRequired: "Введіть номер телефону у міжнародному форматі.",
    errCodeRequired: "Введіть код з Telegram.",
    errPasswordRequired: "Введіть 2FA пароль.",
    errStartFailed: "Не вдалося ініціювати авторизацію.",
    errVerifyFailed: "Не вдалося завершити авторизацію.",
    errDisconnectFailed: "Не вдалося відключити сесію.",
    errCredsRequired: "Заповніть обидва поля: API ID + API Hash.",
    errCredsSaveFailed: "Не вдалося зберегти ключі.",
    errInvalidApiId:
      "API ID має бути додатним числом (наприклад 12345678).",
    errInvalidApiHash:
      "API HASH має бути hex-рядком 32 символи (a-f, 0-9).",
    errNoCredentials: "Спочатку збережіть Telegram API ключі.",
    errCredsNotFound: "Не знайдено збережених API ключів.",
    credsAccordion: "Як отримати API ключі (2 хв)",
    credsStep1:
      "1. Відкрий <link>my.telegram.org/apps</link> — увійди номером телефону, на який зареєстрований Telegram.",
    credsStep2:
      "2. Якщо це перший раз — Telegram пришле код у твій Telegram-додаток, введи його.",
    credsStep3Header: "3. Заповни форму «Create application»:",
    credsStep3AppTitle:
      "<strong>App title:</strong> будь-яка назва (напр. <em>Flux Leads</em>)",
    credsStep3ShortName:
      "<strong>Short name:</strong> 5+ латинських символів (напр. <em>fluxleads</em>)",
    credsStep3Platform:
      "<strong>Platform:</strong> Web (або інше — не критично)",
    credsStep3UrlDesc: "URL/Description можна лишити порожніми",
    credsStep4:
      "4. Натисни <strong>Create application</strong>. На наступному екрані побачиш <code>api_id</code> (число) та <code>api_hash</code> (32 hex-символи).",
    credsStep5: "5. Скопіюй обидва й встав сюди ↓",
    credsSecurity:
      "⚠️ Ці ключі прив'язані до твого Telegram акаунту. НЕ ділись ними публічно. Ми шифруємо api_hash у БД через AES-256-GCM.",
    apiIdLabel: "API ID",
    apiHashLabel: "API Hash",
    saveCreds: "Зберегти ключі",
    savingCreds: "Зберігаю…",
    phoneInstruction:
      "Крок 2 з 2. Введи номер телефону, на який зареєстрований твій Telegram акаунт.",
    disclaimer:
      "Ми зберігаємо сесію зашифрованою AES-256-GCM. Bot-token не використовується — це MTProto userbot під вашим акаунтом, тож дотримуйтесь правил Telegram (без масового спаму).",
  },
  Autopilot: {
    back: "Назад до панелі",
    title: "Автопілот",
    subtitle:
      "Налаштуйте повторюваний пошук лідів і автоматичну розсилку. Cron запускає конвеєр щодня: пошук → аудит → AI-лист → доставка.",
    telegramHint:
      "Щоб увімкнути канал Telegram, спершу підключіть номер у <link>Налаштуваннях</link>.",
    modeName: {
      LOCAL: "Локальний бізнес",
      BEATS: "Артисти (біти)",
      UNIVERSAL: "Універсальний AI",
    },
    searchQueryLabel: "Пошуковий запит",
    searchQueryHint:
      "Ніша / посада / жанр. Підставляється у промпт Gemini з Google Search grounding.",
    searchQueryPlaceholder: {
      LOCAL: "Зубні клініки",
      BEATS: "Trap артисти 100k+ фоловерів",
      UNIVERSAL: "SaaS founders looking for new SDR",
    },
    targetRegionLabel: "Регіон",
    targetRegionHint:
      "Регіон фокусу для AI: країна, місто або «Global». Можна залишити пустим.",
    targetRegionPlaceholder: "Poland, London, Global…",
    outputLanguageLabel: "Мова листа",
    outputLanguageHint:
      "Якою мовою Gemini генерує текст outreach. На пошук не впливає.",
    language: {
      English: "Англійська",
      Ukrainian: "Українська",
      Polish: "Польська",
      German: "Німецька",
    },
    maxPerDayLabel: "Максимум лідів на день",
    maxPerDayHint: "Обмеження конвеєра — щоб не вичерпати ліміт плану одним cron-запуском.",
    channelsLabel: "Канали доставки",
    channelsHint:
      "Хоча б один канал має бути активний, інакше доставка не запуститься.",
    channelEmail: "Email (через SMTP)",
    channelTelegram: "Telegram (через ваш userbot)",
    telegramNotReady: "Підключіть Telegram у Налаштуваннях",
    activeLabel: "Активний",
    activeHint: "Якщо вимкнено — cron пропускає цей конфіг.",
    save: "Зберегти",
    saving: "Зберігаю…",
    delete: "Видалити",
    confirmDelete:
      "Видалити цей конфіг автопілота? Існуючі ліди залишаться, але cron більше не буде створювати нові.",
    infoSaved: "Збережено.",
    infoDeleted: "Видалено.",
    errSaveFailed: "Не вдалося зберегти.",
    errToggleFailed: "Не вдалося перемкнути активність.",
    errDeleteFailed: "Не вдалося видалити.",
  },
} satisfies AbstractIntlMessages;

export default messages;
